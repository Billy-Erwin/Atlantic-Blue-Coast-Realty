import categories from "../assets/files/customCategoryList.json";
...
export class Listing {
        ...
        for (let mainCategory of mainCategories) {
          ...
          for (let subCategory of subCategories) {
            ...
            for (let customField of customFields) {
              ...
              customList[mainCategory][subCategoryName][fieldNames] =
  ...
  getStreetAddress(): string {
    let streetDirPrefix =
      this.standardFields["StreetDirPrefix"] != null
        ? this.standardFields["StreetDirPrefix"]
        : "";
    let streetDirSuffix =
      this.standardFields["StreetDirSuffix"] != null
        ? this.standardFields["StreetDirSuffix"]
        : "";
    let streetName =
      this.standardFields["StreetName"] != null
        ? this.standardFields["StreetName"]
        : "";
    let streetNumber =
      this.standardFields["StreetNumber"] != null
        ? this.standardFields["StreetNumber"]
        : "";
    let streetSuffix =
      this.standardFields["StreetSuffix"] != null
        ? this.standardFields["StreetSuffix"]
        : "";
  ...
  getCityAndState(): string {
    let city =
      this.standardFields["City"] != null ? this.standardFields["City"] : "";
    let stateOrProvince =
      this.standardFields["StateOrProvince"] != null
        ? this.standardFields["StateOrProvince"]
        : "";
  ...
  getBasicDetails(): string {
    let beds =
      (this.validateField(this.standardFields["BedsTotal"])
        ? this.standardFields["BedsTotal"]
        : 0) + " Bedrooms";
    let baths =
      (this.validateField(this.standardFields["BathsFull"])
        ? this.standardFields["BathsFull"]
        : 0) + " Baths";
    let sqft =
      (this.validateField(this.standardFields["BuildingAreaTotal"])
        ? this.standardFields["BuildingAreaTotal"]
        : 0) + " Sqft";
    let yearBuilt =
      "Built in " +
      (this.validateField(this.standardFields["YearBuilt"])
        ? this.standardFields["YearBuilt"]
        : "");
    return `${beds} ${baths}
		${sqft} ${yearBuilt}`;
  }
    ...
    if (this.customFields && this.customFields["Main"]) {
      for (let customFieldKey of this.customMain) {
        this.detailSections.set(
          customFieldKey,
          this.customFields["Main"][customFieldKey]
        );
      }
