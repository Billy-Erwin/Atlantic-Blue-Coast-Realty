...
import searchOptions from "../../assets/files/advanced_search_options.json";
...
export class AbcAdvancedSearchComponent implements OnInit, OnDestroy {
  ...
  onSubmit(page) {
    ...
    this.listingsService
      .getFilteredListings(this.model.filterString, this.model.searchText, page)
      .subscribe((data) => {
    ...
    if (minValue != null) {
      if (minValue == "Any") {
        this.advancedSearchOptions[maxList] =
          searchOptions["default"][defaultListName];
      } else {
        this.advancedSearchOptions[maxList] = [];
